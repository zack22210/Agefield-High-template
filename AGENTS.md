...
- 收到新游戏名和域名后立即开始，不等待人工审核、方案确认或逐步批准。只有缺少必需的用户输入或凭据时才暂停。
- 新 Wiki 站的资料研究、事实核验、内容规划、本地文件编辑和质量验证全部由当前 Codex agent 完成，不调用外部 AI 助手分担建站工作。
- UI 质量优先。不得读取、调用或依赖 `impeccable` skill；所有 UI/UX、主题、布局、响应式和视觉问题均由当前 Codex agent 直接审核并修改。
- 单个新站以尽量在一个 5 小时使用额度内完成为目标。优先集中完成检索、复用模板与自动化验证，不做无关重构、重复调研或人工审核轮次。
- “不用审核”仅指不等待人工复核；由当前 Codex agent 执行的有界视觉检查、自动化质量检查、链接验证、TypeScript 检查和本地构建不得跳过。
...
3. 将结果写入 `站点数据采集目录/关键词分类.json`。
4. 运行 `pnpm seoscout:run`。该命令会自动准备 `keywords.json`、写入游戏名/官网到提示词、同步官方域名，再执行 `seoscout run --keywords keywords.json`（搜索、采集、英文 MDX、翻译），然后做质量检查与内容同步，中途不等待人工审核。
