...
- 默认自动使用用户已登录的 Google AI Pro，通过浏览器调用 Gemini Deep Research 分担阶段 A 的公开资料调查；用户不需要重复说明“使用 Gemini”。Gemini 只负责来源发现、事实候选、语言市场线索和结构化研究草稿，当前 Codex agent 负责核验来源、编辑本地文件和完成构建。
- Gemini 输出不是资料真相源。只采用可访问且符合本文件来源政策的页面，并把核验后的内容写入 `站点数据采集目录/` 现有真相源；不要把长篇 Gemini 原始回答直接塞入仓库或 Codex 上下文。
- 为节省 Codex 额度，要求 Gemini 输出紧凑的结构化结果：每条事实附直接来源 URL，区分已确认、冲突和未知，排除竞对 Wiki、聚合 Wiki、作弊、盗版和风险来源，不生成文章正文。
- 阶段 B 收到原始关键词后，默认让 Gemini 辅助生成去重、同义词合并、搜索意图分组和风险词标记的草案；最终分类仍由当前 Codex agent 按本文件规则决定。SEOScout 的批量文章生成与翻译继续使用项目配置的 LLM API，不使用 Google AI Pro 网页订阅冒充 API。
- SEOScout 生成完成后，可以让 Gemini 对照已核验资料返回精简的问题清单，辅助抽查事实、翻译遗漏、重复和过时内容；不要让它绕过本地质量门禁，也不要用它批量重写全部文章。
- 自动调用 Google AI Pro 的前提是可控浏览器中已有可用的 Google 登录会话。只有登录失效、Google 要求验证码/人工确认、相关权益在账号地区不可用或浏览器连接不可用时才暂停并请用户处理；不得索要用户密码、验证码或完整 API 密钥。
- UI 质量优先。所有 UI/UX、主题、布局、响应式和视觉修改必须使用 `impeccable` skill，并由当前 Codex agent 直接完成。
- 单个新站以在一个 5 小时使用额度内完成为目标。优先并行可独立任务、复用模板与自动化验证，不做无关重构、重复调研或人工审核轮次。
- “不用审核”仅指不等待人工复核；`impeccable` 规定的有界视觉检查、自动化质量检查、链接验证、TypeScript 检查和本地构建不得跳过。
...
4. 运行 `pnpm seoscout:run`，连续完成搜索、采集、英文 MDX、最多三种非英文语言翻译、质量检查与内容同步，中途不等待人工审核。
...
- SEOScout 为每个搜索意图采集 YouTube 字幕时，只取过滤后排名最靠前的 1–2 个可用视频，不得为同一意图扩展采集更多视频。
