...
- 默认自动通过已登录的 Google AI Pro 调用 Gemini Deep Research，分担公开资料发现、语言市场线索和结构化研究草稿；用户不需要另行下达 Gemini 指令。
- Gemini 研究结果必须保持紧凑并为每条事实附直接来源 URL。它只作为研究草稿，Codex 仍需按来源政策核验后才能写入 `站点数据采集目录/`。
- 原始关键词到位后，Gemini 默认辅助去重、合并同义意图、初步分组和风险标记；Codex 负责最终分类。Google AI Pro 网页订阅不代替 SEOScout 所需的批量 LLM API。
- Gemini 还可以在生成后对照已核验资料输出精简问题清单，但不能替代本地质量检查。只有 Google 登录、验证码、地区权益或浏览器连接阻塞时才请求用户介入。
- UI 质量优先；UI 和视觉调整必须使用 `impeccable`，并由当前 Codex agent 直接完成。
- 单站目标是在一个 5 小时使用额度内完成，但仍保留自动化质量门禁和本地构建验证。
