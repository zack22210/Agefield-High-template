...
No secret is currently required to build the public wiki. `NEXT_PUBLIC_SITE_URL` is the only recommended deployment variable; SEOScout API keys are generation-time inputs and are not required by `pnpm build`. Adsterra keys stay empty in this template. Copy `.env.example` to `.env.local`, fill `NEXT_PUBLIC_AD_*`, then `pnpm ads:sync` writes `public/ads/banner-*.html`. See `public/ads/README.md`.
